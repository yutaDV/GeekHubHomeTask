"""НT14. 1 Додайте до банкомату меню отримання поточного курсу валют за допомогою requests
...
class Currency:
    """A class that displays currency rates for the current date"""
...
def welcome_menu():
    ...
    menu = {
        ...
        'exit': 'Exit',
        'cur': 'NEW For looking currency rates'
    }
    print('\n************************WELCOME TO OURS ATM *************************************')
    time.sleep(1)
    print('\nChoice of action')
...
def start():
    ...
    while True:
        ...
        if next_step == 'cur':
            parser = Currency()
            page = parser.get_currency()
            print(parser.return_result(page))
        if next_step == 'exit':
            return "Good bye"
        if next_step == 'reg':
            while True:
